    ...
    return (
        <div className="min-h-screen bg-bg-page pt-32 pb-20 px-4 sm:px-6 lg:px-8 overflow-hidden relative">
            ...
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-7xl h-full pointer-events-none">
                <div className="absolute top-20 left-10 w-72 h-72 bg-blue-100/50 dark:bg-blue-900/20 rounded-full blur-[100px] opacity-60" />
                <div className="absolute top-40 right-10 w-96 h-96 bg-purple-100/50 dark:bg-purple-900/20 rounded-full blur-[120px] opacity-60" />
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-10 max-w-7xl mx-auto">
                    {plans.map((plan, index) => {
                        ...
                        return (
                            ...
                            >
                                {isPopular && (
                                    <div className="absolute -top-5 left-1/2 -translate-x-1/2 bg-blue-600 dark:bg-blue-500 text-white px-8 py-2.5 rounded-full text-[10px] font-black uppercase tracking-[0.2em] shadow-xl shadow-blue-600/30">
                                        Best Value
                                    </div>
                                )}
                                ...
                                <h3 className="text-3xl font-black text-text-primary leading-tight">{plan.name}</h3>
                                <div className="mt-8 flex flex-col">
                                    <div className="flex items-baseline gap-1">
                                      <span className="text-6xl font-black text-text-primary">৳{plan.price}</span>
                                      <span className="text-text-muted font-bold text-lg">/{plan.duration.split(' ')[1] || 'mo'}</span>
                                    </div>
                                    <p className="text-text-muted font-bold uppercase tracking-widest text-[10px] mt-4">Subscription Plan</p>
                                </div>
