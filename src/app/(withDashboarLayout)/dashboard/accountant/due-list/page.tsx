    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="flex items-center justify-between">
        ...
        <button 
          onClick={loadData}
          disabled={isLoading}
          className="p-2 bg-white border border-slate-200 hover:bg-slate-50 rounded-xl transition-all disabled:opacity-50 shadow-sm"
        >
          {isLoading ? <Loader2 size={20} className="animate-spin" /> : <Filter size={20} className="text-slate-600" />}
        </button>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                ...
                <th className="px-6 py-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Class / Roll</th>
                <th className="px-6 py-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Pending Items</th>
                <th className="px-6 py-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Oldest Due</th>
                <th className="px-6 py-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Payable</th>
                <th className="px-6 py-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Due Days</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {isLoading ? (
                ...
                filtered.map((student, idx) => (
                    <tr key={student.dbId} className="hover:bg-slate-50/50 transition-colors group">
                        <td className="px-6 py-4">
                          <p className="text-sm font-bold text-slate-900 group-hover:text-blue-600 transition-colors">{student.name}</p>
                          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">ID: {student.id}</p>
                        </td>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex flex-col gap-1">
                            ...
                            <p className="text-[10px] text-slate-400 font-medium truncate max-w-[150px]" title={student.categories}>
                              {student.categories}
                            </p>
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-4">
                          <span className="text-xs font-bold text-slate-700">{student.month}</span>
                        </td>
                        ...
                        <td className="px-6 py-4">
                          <span className="px-3 py-1 text-[10px] font-black bg-rose-50 text-rose-700 rounded-full border border-rose-100 uppercase tracking-wider">
                            Pending
                          </span>
                        </td>
                    </tr>
