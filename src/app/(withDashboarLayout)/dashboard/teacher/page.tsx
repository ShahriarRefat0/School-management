    ...
    <div className="space-y-8 animate-fadeIn">
        ...
        {stats.map((stat, idx) => (
          ...
          >
            {stat.title === "Today's Attendance" ? (
              <Link href="/dashboard/teacher/attendance" className="flex flex-col gap-4">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl font-bold text-text-primary">Your Classes</h2>
            </div>
          </div>
          ...
          <div className="grid gap-4">
            {data?.assignedClasses?.length > 0 ? (
              data.assignedClasses.map((cls: any, idx: number) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="font-bold text-text-primary group-hover:text-primary transition-colors">{cls.name}</h4>
                      <p className="text-xs text-text-muted font-medium mt-0.5">Click to view attendance</p>
                    </div>
                  </div>
              ...
              <div className="p-8 text-center bg-bg-card border border-border-light rounded-3xl">
                <p className="text-text-muted font-bold text-sm">No classes assigned to you.</p>
              </div>
            )}
          </div>
        </div>
                ...
                data.notices.map((notice: any, idx: number) => (
                  <div key={idx} className="group cursor-pointer">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span className={cn("shrink-0 px-2 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-wider bg-blue-50 text-blue-600")}>
                        {notice.priority}
                      </span>
                    </div>
                    ...
                    {idx !== data.notices.length - 1 && <div className="h-px bg-border-light mt-5"></div>}
                  </div>
                ))
              ) : (
                <p className="text-xs text-text-muted text-center py-4">No recent notices.</p>
              )}
            </div>
          </div>
        </div>
      ...
      {selectedClassForAttendance && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => !isSaving && setSelectedClassForAttendance(null)}></div>
          <div className="bg-bg-card w-full max-w-2xl rounded-3xl p-8 relative z-10 shadow-2xl animate-slideInBottom border border-border-light max-h-[90vh] overflow-y-auto custom-scrollbar">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-xl font-bold text-text-primary">Attendance: {selectedClassForAttendance.name}</h3>
                  <div className="flex items-center gap-2 mt-1">
                    <p className="text-xs text-text-muted font-medium italic">Record for:</p>
                    <input
                      type="date"
                      value={selectedDateForAttendance}
                      onChange={(e) => {
                        setSelectedDateForAttendance(e.target.value);
                        handleOpenQuickAttendance(selectedClassForAttendance, e.target.value);
                      }}
              ...
              <div className="space-y-4">
                <div className="grid gap-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                  {students.map((student) => (
                    <div key={student.id} className="flex items-center justify-between p-3 bg-bg-page/50 rounded-2xl border border-border-light/50">
                      <div className="flex items-center gap-3">
                        <span className="text-[10px] font-bold text-text-muted w-6">{student.rollNo}</span>
                        <span className="text-sm font-bold text-text-primary">{student.firstName} {student.lastName}</span>
                      </div>
                      <div className="flex gap-2">
                        <button
                          ...
                          className={cn(
                            ...
                            attendance[student.id] === 'PRESENT' ? "bg-emerald-500 text-white" : "bg-bg-card text-emerald-600 border border-emerald-100 hover:bg-emerald-50",
                            isDatePast(selectedDateForAttendance) && "opacity-50 cursor-not-allowed"
                          )}
                        >
                        ...
                        <button
                          ...
                          className={cn(
                            "p-2 rounded-lg transition-all",
                            attendance[student.id] === 'ABSENT' ? "bg-red-500 text-white" : "bg-bg-card text-red-600 border border-red-100 hover:bg-red-50",
                            isDatePast(selectedDateForAttendance) && "opacity-50 cursor-not-allowed"
                          )}
                        >
                          <XCircle size={16} />
                  ...
                  {students.length === 0 && <p className="text-center py-10 text-text-muted">No students in this class.</p>}
                </div>
                ...
                <div className="flex gap-3 pt-4">
                  ...
                  <button
                    disabled={isSaving || students.length === 0 || isDatePast(selectedDateForAttendance)}
                  ...
                  >
                    ...
                    {isSaving ? 'Saving...' : isDatePast(selectedDateForAttendance) ? 'Read Only' : 'Save Attendance'}
                  </button>
                </div>
              </div>
            )}
