...
const AllExamsByTheTeacher = () => {
  ...
  useEffect(() => {
    if (user?.email) {
      fetchExams(user.email.toLowerCase());
    } else {
      setLoading(false);
    }
  }, [user]);
  ...
  const fetchExams = async (email) => {
    try {
      const res = await fetch(`/api/exams/create?email=${encodeURIComponent(email)}`);
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      ...
      if (data.success) {
        setExams(
          exams.map((exam) =>
            String(exam._id) === String(id) ? { ...exam, exam: newStatus } : exam,
          ),
        );
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        const data = await res.json();
        if (data.success) {
          setExams(exams.filter((exam) => String(exam._id) !== String(id)));
