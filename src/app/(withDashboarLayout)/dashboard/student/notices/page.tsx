...
import { Calendar, Tag, ChevronRight, Search, FileText } from "lucide-react"
...
export default function NoticesPage() {
    ...
    useEffect(() => {
        async function loadNotices() {
            ...
            if (res.success) {
                // Show all published notices
                const allNotices = (res.data || []).filter((n: any) =>
                    n.status === "published" &&
    ...
    const filteredNotices = notices.filter(n => {
        ...
        const noticeCategory = n.category
            ?.toLowerCase()
            ?.replace(/\s+/g, "-")   // space → dash

        const matchesCategory =
            selectedCategory === "All" ||
            noticeCategory === selectedCategory.toLowerCase()
                ...
                {loading ? (
                    ...
                    filteredNotices.map((notice) => (
                        <div key={notice.id} className="group bg-white border border-slate-200 rounded-xl p-5 shadow-sm hover:shadow-md transition-all cursor-pointer relative overflow-hidden">
                            <div className={`absolute left-0 top-0 bottom-0 w-1 ${notice.priority === 'urgent' ? 'bg-red-500' :
                                notice.priority === 'high' ? 'bg-red-500' :
                                    notice.priority === 'normal' ? 'bg-blue-500' : 'bg-amber-500'
                                }`}></div>
                                ...
                                <div className="space-y-2 flex-1">
                                    <div className="flex items-center gap-3">
                                        <span className={`
                                            ...
                                            ${notice.category?.toLowerCase() === 'academic' ? 'bg-blue-100 text-blue-700' :
                                                notice.category?.toLowerCase() === 'event' ? 'bg-purple-100 text-purple-700' :
                                                    notice.category?.toLowerCase() === 'emergency' ? 'bg-red-100 text-red-700' :
                                                        notice.category?.toLowerCase() === 'exam' ? 'bg-emerald-100 text-emerald-700' :
                                                            'bg-slate-100 text-slate-700'}
                                        `}>
                                            {notice.category || "General"}
                                        </span>
                                        <span className="flex items-center text-xs text-slate-400 font-medium">
                                            <Calendar className="h-3 w-3 mr-1" /> {new Date(notice.createdAt).toLocaleDateString()}
                                        </span>
                                    </div>
                    ...
                    <div className="text-center py-12 bg-white border border-slate-200 rounded-xl border-dashed">
                        ...
                        <p className="text-slate-500">There are currently no announcements for you.</p>
                    </div>
                )}
            </div>
        </div>
